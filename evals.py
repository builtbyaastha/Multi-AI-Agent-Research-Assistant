...
class JudgeResult(BaseModel):
    criteria_met_count: int = Field(
        description="How many of the listed criteria the report fully satisfies."
    )
    total_criteria: int = Field(
        description="Total number of criteria listed for this query."
    )
    verdict: Literal["pass", "fail"] = Field(
        description="'pass' only if criteria_met_count == total_criteria, otherwise 'fail'."
    )
    reasoning: str = Field(
        description="One or two sentences on which criteria were or weren't met, and why."
    )
    ...
    for item in dataset:
        ...
        if pipeline_ok:
            try:
                ...
                judge_ok = False
                judge_error = str(e)
                print(f"[eval] {item['id']} judge failed: {judge_error}")
        else:
            judgment = None
            judge_ok = False
        ...
        result = {
            ...
            "judge_criteria_met_count": judgment.criteria_met_count if judgment else None,
            "judge_total_criteria": judgment.total_criteria if judgment else None,
